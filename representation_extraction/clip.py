...
import os
from PIL import Image
from transformers import CLIPProcessor, CLIPModel, CLIPTokenizerFast, CLIPTextModel
import argparse
import numpy as np
import pickle
...
if args.context =='picture':
    ...
    images = [Image.open(img) for img in im_paths]
    ...
    img = Image.open(os.path.join(PEREIRA_DIR, path))
...
pickle.dump(embs_to_save, open(ROOT / f"data/embeddings/clip/clip_{args.context}.pkl", "wb"))
